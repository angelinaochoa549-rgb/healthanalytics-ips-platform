...
class ETLPipeline:
    ...
    def transform(self):
        ...
        # Convertir columnas numéricas (texto invalido -> NaN)
        ...
        for col in nums:
            if col in df.columns:
                ...
                invalidos_texto += (despues_na - antes_na)
 
        # Reemplazar outliers numéricos fuera de rango con NaN
        invalidos = 0
        ...
        invalidos += invalidos_texto
        self.log.registros_invalidos = invalidos
        self._log(f"Valores fuera de rango o no numéricos: {invalidos}")
 
        # Imputar nulos con la mediana de la columna (forma segura, sin chained assignment)
        for col in nums:
            if col in df.columns:
